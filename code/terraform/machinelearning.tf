...
data "azurerm_monitor_diagnostic_categories" "diagnostic_categories_machine_learning_workspace" {
  resource_id = azapi_resource.machine_learning_workspace.id
}

resource "azurerm_monitor_diagnostic_setting" "diagnostic_setting_machine_learning_workspace" {
  name                       = "logAnalytics"
  target_resource_id         = azapi_resource.machine_learning_workspace.id
...
resource "azurerm_private_endpoint" "machine_learning_workspace_private_endpoint" {
  name                = "${azapi_resource.machine_learning_workspace.name}-pe"
  location            = var.location
  resource_group_name = azapi_resource.machine_learning_workspace.resource_group_name
  tags                = var.tags

  custom_network_interface_name = "${azapi_resource.machine_learning_workspace.name}-nic"
  private_service_connection {
    name                           = "${azapi_resource.machine_learning_workspace.name}-pe"
    is_manual_connection           = false
    private_connection_resource_id = azapi_resource.machine_learning_workspace.id
  ...
  dynamic "private_dns_zone_group" {
    ...
    content {
      name = "${azapi_resource.machine_learning_workspace.name}-arecord"
